...
export const CountryPage = () => {
	...
	const { data } = useCountry(params.code as string);
	const { image, isLoading } = useImage(data?.country.name as string);
		...
		<section className='max-w-4xl w-full flex flex-col gap-4 mx-auto py-16 px-4 relative'>
			<header className='w-full space-y-4'>
				{isLoading ? (
					...
					<img
						className='w-full h-40 object-cover object-top rounded-sm'
						src={image}
